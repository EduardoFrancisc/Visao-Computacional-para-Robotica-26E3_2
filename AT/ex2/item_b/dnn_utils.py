"""
UTILITÁRIOS DOS EXEMPLOS

Este arquivo concentra funções repetidas para que os exemplos fiquem mais didáticos.
O aluno não precisa começar por aqui; a recomendação é executar os exemplos em ordem.
"""
...
def hog_or_haar_detector(frame):
    ...
    try:
        ...
        print("Aviso: HOG falhou. Verifique se o pacote opencv-python está instalado corretamente.")

    # 2. Detecção de Faces (Haar Cascade)
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    
    # Aponta para a pasta onde este script (dnn_utils.py) está salvo
    pasta_atual = os.path.dirname(os.path.abspath(__file__))
    cascade_path = os.path.join(pasta_atual, 'haarcascade_frontalface_default.xml')
    
    face = cv2.CascadeClassifier(cascade_path)
    
    # Verifica se o arquivo foi carregado com sucesso antes de detectar
    if face.empty():
        print(f"Aviso: Arquivo XML do Haar Cascade não encontrado em:\n{cascade_path}")
    else:
